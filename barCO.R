...
taxonomy <- sapply(strsplit(data_raw$taxonomy, "; g__"), function(x) x[length(x)])
rownames(data_raw) <- taxonomy
# 删除第一列OTU
data <- data_raw[,-1]
# 删除最后一列taxonomy
...
####修改分组信息对应列数####
control_rows <- map %>%  
  filter(Category1 == "CONTROL")  

# 如果你需要这些行的行名  
control_rownames <- rownames(control_rows)  

osahs_rows <- map %>%  
  filter(Category1 == "OSAHS")  

# 如果你需要这些行的行名  
osahs_rownames <- rownames(osahs_rows)
data3 <- data3 %>%  
  mutate(CONTROL = rowMeans(data3[, c(control_rownames)]),OSAHS=rowMeans(data3[, c(osahs_rownames)]))

data3 <- data3[,c('CONTROL','OSAHS')]
...
# 显示条形图
p
ggsave("0828.CO.bar.pdf", egg::set_panel_size( p, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)

data3$diff <- data3$OSAHS-data3$CONTROL 
df_sorted <- data3 %>%  
  arrange(desc(diff))  
write.table(df_sorted,file = 'species.top20diff.csv',sep = ',')
...
# ggsave("0828.CO.diff.bar.pdf",p,width = 6, height = 8)
# dev.off()
...
p <- ggplot(df_sorted, aes(x=Taxonomy, y=diff)) +   
  ...
  theme_minimal() +  
  labs(title="diff", x="", y="O-C")+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +  
  # 将x轴标签旋转90度并调整其大小  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 12))
ggsave("0828.CO.diff.bar.pdf",p,width = 4, height = 6)
...
# 显示条形图
p_Phylum
ggsave("0828.Phylum.CO.bar.pdf", egg::set_panel_size( p_Phylum, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)
...
# 显示条形图
p_Class
ggsave("0828.Class.CO.bar.pdf", egg::set_panel_size( p_Class, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)
...
# 显示条形图
p_Order
ggsave("0828.Order.CO.bar.pdf", egg::set_panel_size( p_Order, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)
...
# 显示条形图
p_Family
ggsave("0828.Family.CO.bar.pdf", egg::set_panel_size( p_Family, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)
...
# 显示条形图
p_Genus
ggsave("0828.Genus.CO.bar.pdf", egg::set_panel_size( p_Genus, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)
...
# 显示条形图
p_Species
ggsave("0828.Species.CO.bar.pdf", egg::set_panel_size( p_Species, width=unit(1, "in"), height=unit(5, "in")), 
       width = 8, height = 7, units = 'in', dpi = 300)
