...
df_summarised <- df_split %>%  
  group_by(col2) %>%  
  summarise(  
    across(ends_with(".bracken.S"), ~ sum(.x, na.rm = TRUE), .names = "{col}")  
  )
...
#####查看物种是否存在####
values1 <- df_summarised[df_summarised$col2 == " p__Bacteroidota", -1]
values2 <- df_summarised[df_summarised$col2 == " p__Proteobacteria", -1]
values3 <- df_summarised[df_summarised$col2 == " p__Firmicutes", -1] 
values3 <- df_summarised[df_summarised$col2 == " p__Gammaprote", -1] 
original_colnames <- colnames(values1) 
cleaned_colnames <- gsub(".bracken.S", "", original_colnames)  
# 将修改后的列名赋值回数据框  
colnames(values1) <- cleaned_colnames
rownames(values1) <- c('p__Bacteroidota')
values1 <- t(values1)
...
# 设置每个组的颜色
group_colors <- c("C" = "#BD3C29", "O" = "#78D3AC")
# 首先设置比较的列表
compare_list <- list(
  c("C","O"))

# 使用ggplot2创建Shannon箱线图
p_values1 <- ggplot(plot_data, aes(x = Group, y = p__Bacteroidota, fill = Group, colour = Group)) +
  geom_boxplot(width = 0.5, alpha = 0.6, lwd = 1.15, outlier.shape = NA) +  # 调整箱的大小
  geom_jitter(width = 0.3, size = 3, alpha = 0.75) +  # 添加散点
  labs(y = "p__Bacteroidota") +
  scale_fill_manual(values = group_colors) +  # 设置颜色
  scale_color_manual(values = group_colors) +
  theme_bw() +
  labs(x = NULL) +
...
#print(p)
# 保存图形为PDF
ggsave("0613.p__Bacteroidota.all.pdf", p_values1, height = 5, width = 5)