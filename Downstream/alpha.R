...
library(ggplot2)
library(dplyr)
otu_table  <-  read.delim("./0828.S.count.tsv.all.tmp", header=T, sep="\t", row.names=1, stringsAsFactors = FALSE)
...
#print(p)
# 保存图形为PDF
ggsave("0828.wilcox.Richness.all.pdf", p_Richness, height = 5, width = 5)
...
#print(p)
# 保存图形为PDF
ggsave("0828.wilcox.Shannon.all.pdf", p_Shannon, height = 5, width = 5)
...
# 保存图形为PDF
ggsave("0828.wilcox.Simpson.all.pdf", p_Simpson, height = 5, width = 5)
...
#print(p)
# 保存图形为PDF
ggsave("0828.t.Simpson.all.pdf", p_tSimpson, height = 5, width = 5)
