...
p_volcano <- EnhancedVolcano(
  ...
  #   sprintf("%s < %s    and   |log2FC| >= %s", pcol, pvalue, log2fc)
  # )
) + coord_cartesian(ylim = c(0,10))+ scale_y_continuous(expand = c(0, 0.1, 0.1, 0.1))+theme(plot.title = element_text(hjust = 0.5)) # 设置图例标签 # 调整y轴的范围
...
df_up <- df_up %>%  
  mutate(change = "UP")  
write.table(df_up, file = "HYXM.all.species.up.csv", sep = ",", row.names = FALSE, quote = FALSE, na = "")
...
df_down <- df_down %>%  
  mutate(change = "DOWN")
write.table(df_down, file = "HYXM.all.species.down.csv", sep = ",", row.names = FALSE, quote = FALSE, na = "")

change <- bind_rows(df_up,df_down)
write.table(change, file = "HYXM.all.species.change.csv", sep = ",", row.names = FALSE, quote = FALSE, na = "")
# df_downf
...
write(variables, file = "0828.all.variables.txt") 
...
index = ( grepl("Eubacterium", df_s_up$Features)|grepl("Barnesiella", df_s_up$Features)|grepl("Escherichia", df_s_up$Features)|grepl("Shigella", df_s_up$Features)|grepl("Lactobacillus", df_s_up$Features))
# df_a = df[index,]
...
grid_plot <- plot_grid(plotlist =p_list , nrow = 2,ncol =4)
grid_plot
ggsave(
  plot = grid_plot,
  "0828.all.ge.pdf",
...
# )
# 提取前20名的微生物丰度数据
abundance <- abundance[, c(df_upf[1:10],df_downf[1:10]), drop = FALSE]
...
pheatmap(
  df_log,  # 丰度数据
  file = "0828.all.heatmap.top10.pdf",  # 输出文件名
